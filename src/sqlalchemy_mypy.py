...
def decl_info_hook(ctx: DynamicClassDefContext) -> None:
    ...
    obj = (
        ctx.api.builtin_type("builtins.object")
        if hasattr(ctx.api, "builtin_type")
        else ctx.api.named_type("builtins.object")
    )
...
def model_hook(ctx: FunctionContext) -> Type:
    ...
    for actual_name, actual_type in zip(ctx.arg_names[0], ctx.arg_types[0]):
        ...
        ctx.api.check_subtype(
            ...
            ctx.context,
            f'Incompatible type for "{actual_name}" of "{shortname(model)}"',
            "got",
            "expected",
        )
    return ctx.default_return_type
...
def relationship_hook(ctx: FunctionContext) -> Type:
    ...
    if isinstance(arg, StrExpr):
        ...
        if sym and isinstance(sym.node, TypeInfo):
            ...
            ctx.api.fail(f'Cannot find model "{name}"', ctx.context)
